...
%token FINAL_EOF DELIMETER CURLY_OPEN CURLY_CLOSE ASSIGN 
%token MAIN WRITE READ SIZE PLOT RETURN
...
if: IF '(' expression ')' CURLY_OPEN block if_end {$$ = add_if_node($3, add_block_node($6), $7); };

while: WHILE '(' expression ')' CURLY_OPEN block CURLY_CLOSE { $$ = add_while_node($3, add_block_node($6)); };
